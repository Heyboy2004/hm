...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{file.file_name}", url=await get_search_results(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
                InlineKeyboardButton(
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        button = [[
            ...
            InlineKeyboardButton('🔍 release date 🔍', url=f"https://www.google.com/search?q={reqst_gle}+release+date")
            ]]        
            k=await query.message.edit('<b>✯ നിങ്ങൾ ചോദിച്ച മൂവി റിലീസ് ആയിട്ടുണ്ടോ..? </b>\n\n✯ 𝗜𝗳 𝗶𝘁 𝗶𝘀 , 𝗪𝗲 𝗪𝗶𝗹𝗹 𝗨𝗽𝗹𝗼𝗮𝗱 𝗜𝘁⚡️\n\n<b>📯ɴʙ: Cʟɪᴄᴋ Bᴇʟᴏᴡ Rᴇᴀsᴏɴs Bᴜᴛᴛᴏɴ</b>', reply_markup=InlineKeyboardMarkup(button))    
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('𖣘 𝗠𝗢𝗩𝗜𝗘𝗦 𝗚𝗥𝗢𝗨𝗣 𖣘', url='https://t.me/color_films')
         ]]
        try:
...
async def advantage_spell_chok(client, msg):
    ...
    if not movies:
        ...
        button = [[
            InlineKeyboardButton('📕 ɪɴsᴛʀᴜᴄᴛɪᴏɴ 📕', callback_data='reqst11')
            ],[   
